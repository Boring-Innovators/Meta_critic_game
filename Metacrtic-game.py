...
with open("Add_Games_Here/meta_names.mtac", "r") as file:
    names = file.readlines()
    game_list = [name.strip() for name in names]

config_list = []
with open("config.txt", "r") as file:
    names = file.readlines()
    config_list = [name.strip().replace("CHARACTER_LIMIT = ", "") for name in names]

headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3'}
char_limit = float(config_list[0])
review_limit = 3
            ...
            if blurb:
                ...
                if detect(text) == "en" and text not in used_reviews and len(text) <= char_limit:
                    if score:
                        ...
                        if 0 <= score_value <= 3 or 9 <= score_value <= 10:
...
def generateBoard(color):
    ...
    return realgame_rect, fakegame1_rect, fakegame2_rect
...
pygame.display.set_caption("Metacritic game : The Boring Innovators")

icon_surface = pygame.image.load("Add_Games_Here/MetImage.png").convert_alpha()
...
realgame_rect, fakegame1_rect, fakegame2_rect = generateBoard(color)
running = True
while running:
